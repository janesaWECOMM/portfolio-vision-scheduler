...
const TestimonialsSection = () => {
  ...
  const renderTestimonialCard = (testimonial: Testimonial, index: number) => (
    <div className="glass rounded-xl p-6 md:p-8 relative overflow-hidden h-full">
      {/* Stars aligned horizontally */}
      <div className="flex text-boost-orange mb-6">
        {Array.from({ length: testimonial.rating }).map((_, i) => (
          <Star key={i} size={16} className="inline-block fill-current mr-1" />
        ))}
      </div>
      
      <blockquote className="text-base md:text-xl mb-6 italic min-h-[100px]">
        "{testimonial.content}"
      </blockquote>
      
      <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 mt-auto">
        {testimonial.companyLogo && (
          <img 
            src={testimonial.companyLogo} 
            alt={testimonial.company} 
            className="h-8 md:h-12 md:mr-4 mb-2 md:mb-0" 
          />
        )}
        <div>
  ...
  return (
    <section 
      ref={sectionRef} 
      className="py-16 md:py-24 px-4 bg-gradient-to-b from-white to-boost-purple/5 opacity-0" 
      style={{
          ...
          {isMobile ? (
            <Carousel className="w-full">
              ...
              <div className="flex items-center justify-center mt-6 gap-2">
                <CarouselPrevious className="static translate-y-0 transform-none mx-1" />
                <CarouselNext className="static translate-y-0 transform-none mx-1" />
              </div>
              ...
              <div className="mt-4 flex justify-center space-x-2">
                {testimonials.map((_, index) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${index === currentIndex ? "w-8 bg-boost-purple" : "w-2 bg-muted"}`}
            ...
            <>
              <div className="glass rounded-xl p-6 md:p-12 relative overflow-hidden">
                ...
                <div className="mt-6 md:mt-0 md:absolute md:right-8 md:bottom-8 flex justify-center space-x-2">
                  <Button variant="outline" size="icon" onClick={() => setCurrentIndex((prev) => (prev - 1 + testimonials.length) % testimonials.length)} className="rounded-full h-10 w-10">
                    <ArrowLeft size={18} />
                  </Button>
                  <Button variant="outline" size="icon" onClick={() => setCurrentIndex((prev) => (prev + 1) % testimonials.length)} className="rounded-full h-10 w-10">
                    <ArrowRight size={18} />
                  </Button>
                </div>
              </div>
