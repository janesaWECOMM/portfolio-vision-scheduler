...
const FeatureCard = ({
  ...
  description,
  delay
}: FeatureCardProps) => <div className="glass rounded-xl p-5 transition-all duration-300 hover:shadow-lg opacity-0 animate-fade-in flex-shrink-0 w-[280px]" style={{
...
const FeaturesSection = () => {
  ...
  return <section className="py-24 px-4" ref={sectionRef}>
      <div className="container mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-12 opacity-0 animate-fade-in" style={{
        ...
        animationFillMode: 'forwards'
          ...
          <p className="text-xl text-muted-foreground">Experience the difference with our specialised programs designed for modern professionals.</p>
        </div>

        <div className="overflow-x-auto pb-6 -mx-4 px-4">
          <div className="flex space-x-5 w-max">
            {features.map((feature, index) => <FeatureCard key={index} icon={feature.icon} title={feature.title} description={feature.description} delay={feature.delay} />)}
          </div>
        </div>
      </div>
    </section>;
